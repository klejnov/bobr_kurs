...
function bps_sberbank_by($banks_id)
{
    ...
    if (preg_match("/Доллар США < 200.*?Покупка<\/div><\/div>([^<]*)<.*?Продажа<\/div><\/div>([^<]*)<.*?Eвро < 200.*?Покупка<\/div><\/div>([^<]*)<.*?Продажа<\/div><\/div>([^<]*)<.*?Российские рубли.*?Покупка<\/div><\/div>([^<]*)<.*?Продажа<\/div><\/div>([^<]*)</ms",
        $html, $valuta)) {
        $status = 1;
